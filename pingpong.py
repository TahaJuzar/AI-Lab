print('k')
import turtle  #Sets up the game window using the turtle graphics library
import random
import numpy as np
window = turtle.Screen()
window.setup(width=800, height=600)
window.tracer(0)
class aipaddleright(turtle.Turtle): #this will make right side paddle that is controlled by AI
    def __init__(self,*args,**kwargs):
        super(aipaddleright,self).__init__(*args,**kwargs)
        self.speed(0)
        self.shape("square")
        self.color("green")
        self.shapesize(stretch_wid=5, stretch_len=1) #it will create a rectangular paddle
        self.penup()
...
#the ball
ball = turtle.Turtle() #it will represent the ball in the game
ball.speed(0)
ball.shape("circle")
ball.color("red") #identifies the red color of the ball
ball.penup()
ball.goto(200,0)
...
player_2_score = 0
#Pen for the scores
pen = turtle.Turtle() #it is for scoring of the player and it will update the score after passing to the paddle
pen.speed(0)
pen.color("black")
pen.penup()
pen.hideturtle()
...
while True: #main game loop
    ...
    ball.sety(multiplyratio*ballydirection+ball.ycor()) #it will move the ball based on its speed and direction
